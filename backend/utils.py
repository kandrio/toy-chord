...
def insert_node_to_ring(hash_id: str, node_ip: str, node_port: int):
    """Add Node to the ring network by sending a request to BOOTSTRAP."""
    bootstrap_url = "http://" + bootstrap_ip + ":" + \
        str(bootstrap_port) + "/bootstrap/node/join"
...
def get_data_from_next_node(node_ip: str , node_port: int):
    ...
    next_node_url = "http://" + node_ip + \
        ":" + str(node_port) + "/node/sendyourdata"
...
def call_update_data(data, my_id, next_ip, next_port):
    ...
    data['start_id'] = my_id
    data['k'] = K_replicas-1
    url_next = "http://" + next_ip + ":" + \
        str(next_port) + "/node/update/data/join"
